...
import {
  missionFormTokenExpiresInMinutes,
  verifyMissionFormToken,
} from "@/lib/mission-form-token";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const v = verifyMissionFormToken(t);
    if (!v) {
    ...
    return NextResponse.json({
      expiresInMinutes: missionFormTokenExpiresInMinutes(),
      employee: {
    ...
    console.error("[mission-request/prefill]", e);
